...
mod tests {
    ...
    fn forward_initial_payload_prefix_matches_tls_after_vision_unpad_not_uuid() {
        ...
        use crate::vless::vision::{wrap_vision_uplink_block, VisionUnpadState};
        ...
        let mut state = VisionUnpadState::new(user_id);
        let forward_initial_payload = state.unpad(parsed_raw_initial_payload).unwrap();
        assert!(forward_initial_payload.starts_with(&[0x16, 0x03]));
        assert_ne!(
