...
mod tests {
    ...
    fn resolve_xhttp_mode_for_settings_rejects_unknown_mode() {
        ...
        let err =
            resolve_xhttp_mode_for_settings(&unknown, TransportSecurity::Reality).unwrap_err();
    ...
    async fn query_string_does_not_break_path_match() {
        ...
        let response =
            run_accepted_post(settings, "/xhttp?sessionId=abc&seq=1", "example.com").await;
