...
use rust_xray::api::proto::app::stats::command::{GetStatsRequest, QueryStatsRequest};
...
async fn get_stats_returns_value_and_reset_clears_counter() {
    ...
    tokio::spawn(async move {
        ...
        let _ = serve_grpc_on(listener, services, registry_for_server, inbound_users).await;
    });
...
async fn get_stats_missing_name_returns_not_found() {
    ...
    tokio::spawn(async move {
        let inbound_users = Arc::new(InboundUserManagers::new());
        let _ = serve_grpc_on(listener, vec![ApiService::Stats], registry, inbound_users).await;
    });
