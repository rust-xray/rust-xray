...
**Valid REALITY clients still stop at `Unsupported` after partial TLS 1.3
handshake generation** (connect to `dest`, forward the client TLS record, validate
the destination TLS 1.3 ServerHello shape, build `RealityTls13ServerState`,
generate ServerHello + encrypted EncryptedExtensions/Finished records). **This is
expected** — partial TLS 1.3 generation is **experimental and not a working
protocol** (no Certificate, CertificateVerify, client Finished, app-data stream).
By default generated bytes are **not sent** to the client. Set
`RUST_XRAY_EXPERIMENTAL_SEND_PARTIAL_TLS13=1` for packet-inspection-only send.
...
- **Fallback relay** — TCP relay to `dest`/`target` for ordinary and invalid
  clients.
- **Accepted path + partial TLS 1.3 generation (experimental)**
  ...
  - TLS 1.3 state + partial handshake generation
    (`generate_partial_tls13_handshake`: ServerHello record, encrypted
    EncryptedExtensions + Finished)
  - accepted path still returns **`Unsupported`** after generation (not a complete
    handshake; no VLESS handoff)
  - optional experimental send:
    `RUST_XRAY_EXPERIMENTAL_SEND_PARTIAL_TLS13=1` writes generated bytes to the
    client for packet inspection only
...
- **TLS 1.3 server state container** — `RealityTls13ServerState` holds
  ...
  handshake secrets, ServerHello / encrypted record builders
  (`src/reality/tls13/state.rs`).
- **Partial TLS 1.3 generation (experimental, not live protocol)** —
  `generate_partial_tls13_handshake` builds ServerHello + encrypted EE/Finished;
  missing Certificate, CertificateVerify, client Finished, app-data wrapper.
...
- **Full REALITY TLS 1.3 server handshake** — partial generation exists; protocol
  is not complete or interoperable.
