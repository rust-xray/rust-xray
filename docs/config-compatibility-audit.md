...
Raw/TCP network aliases, VLESS `flow=""` / `xtls-rprx-vision`, fallback matrix, TLS cipher matrix, ML-DSA-65 seed runtime (experimental baseline), Happ mux UDP DNS baseline, UDP/Mux/XUDP unsupported probes with TCP regressions, and transport rejection checks — see [compatibility-status.md](./compatibility-status.md), live smoke, and `docs/reality-mldsa65-runtime-baseline.md`.
